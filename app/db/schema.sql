...
DROP TABLE IF EXISTS user_definitions CASCADE;
...
CREATE TABLE user_definitions (
    user_definition_id SERIAL PRIMARY KEY,
    user_id INT NOT NULL REFERENCES users(user_id),
    definition_id INT NOT NULL REFERENCES definitions(definition_id)
);
...
CREATE INDEX idx_user_word_review ON user_word_reviews(user_word_id, review_date);
